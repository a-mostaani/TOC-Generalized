...
err_trig=0.3;
best_rew=24;         %note that there is a connection between best reward c_ucb!
worst_rew=1;
goal_set=[n*n];
...
q_table=0.02*ones(2,n^2,2^bits,5,2^bits);
q=0.02*ones(2,n^2,2^bits,5,2^bits);
                    %qc3_table(n^2,:)=[2,2];
...
for i=1:en
    %learning phase
    [learning_rew(:,i),q,epoch_counter_learn(:,i)]=EoC_corrected_interact_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,goal_set,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,q_table,bsc_p_learn,end_learn_learn,update_tables);
                                                                            %(scen,n,noa,ns,bits,best_rew,~        ,~       ,gamma,~    ,~ ,cs,~ ,~ ,~  ,rew,~       ,counter,q_table,bsc_p      ,end_learn      ,update_tables);
    
    %execution phase
%    [epoch_rew(:,i)]=                              EoC_corrected_interact_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,goal_set,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,q,bsc_p_exec,end_learn_exec,0);
