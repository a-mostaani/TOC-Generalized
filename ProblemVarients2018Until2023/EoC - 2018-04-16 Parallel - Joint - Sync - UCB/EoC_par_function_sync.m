function [rew,qp_table,qc_table,counter]=EoC_par_function_sync(scen,n,noa,ns,bits,best_rew,worst_rew,goal_set,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,end_learn,update_tables)
...
stuck_counter=0;
        ...
        %episode run    
            while 3==3    
                ...
                [ca] = cpolicy_customized_nbits(ps,scen,tau,qc_table,i,ns,end_learn);
                                                                
                %SELECT POSITION ACTION AND UPDATE POSITON STATE
                %select position action 
                [pa] = ppolicy_customized_nbits(ps,cs,scen,tau,qp_table,i,ns,end_learn);
