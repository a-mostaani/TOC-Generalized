...
G = 15; % goal point location (first grid is numbered zero)
NI = 1*10^6; % Number of Iterations
...
%% Zero Initialization
l_num = zeros(NI,K); %Agent location numbers at all iterations
...
for i = 1:NI
    ...
    l_num(i,1) = randsample(legit_locs,1);
    l_num(i,2) = randsample(legit_locs,1);
    l_num(i,3) = randsample(legit_locs,1);
    
    %convert location numbers to location coordinates
    l(i,1,:) = num_to_coord(l_num(i,1), N);
    l(i,2,:) = num_to_coord(l_num(i,2), N);
    l(i,3,:) = num_to_coord(l_num(i,3), N);
end
