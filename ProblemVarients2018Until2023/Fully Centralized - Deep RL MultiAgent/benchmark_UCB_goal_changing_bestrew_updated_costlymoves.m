...
best_rew=10;
%move_cost = -0.00025; % for n=3
%move_cost=-10^(-20);
move_cost = 0;

                    
tau=1.5;     %High temperatures cause all actions to be nearly equiprobable, 
             %whereas low temperatures cause greedy action selections   
...
%                 tau = tau - (tau/4 * (i)/(end_learn*ns));
                [main_pa] = bench_policy_UCB(main_ps,tau,qp_table,N_table,ns,i,ucb_counter,end_learn,best_rew);
                pa=pa_calc(main_pa);
                ...
                elseif ter==1
                    ...
                    if (pa==1 | pa==2 | pa==3 | pa==4) %& (i >= ns*0.5)
                        temp_rew = move_cost;
%                         temp_rew = move_cost *(i)/ns;
                    end
...
%% State aggregation according to actions - agent's point of view
plot(movmean(rew,ns*0.1))
grid minor
pi_star = compute_pi(qp_table, goal_set, n);
pi_star(goal_set-1,:,1)

dom_act_per_loc = zeros(n^2,1);
for k = 1 : n^2
   dom_act_per_loc(k)  = mode(pi_star(k,:,1));
end

ag_state_action = zeros(2^inf_bits, length(s_space));
cnt_i=0;
for i=1:2^inf_bits
    cnt_i=1;
    for j=1:length(s_space)
        if dom_act_per_loc(j)==i
            ag_state_action(i,cnt_i)=j;
            cnt_i=cnt_i+1;
        end
    end
end
ag_states = ag_state_action;
dom_act_per_loc
%save('agreggated_states_n4_g11_infbits3', 'ag_states')

%% State aggregation according to actions - central controller's point of view
plot(movmean(rew,ns*0.1))
grid minor
no_actions = 5;
[~,pi_star] = compute_pi(qp_table, goal_set, n);
...
ag_state_action = zeros(no_actions^2, length(s_space));
act_presence = zeros(rows,1);
cnt_i=0;
for i=1:no_actions^2
    cnt_i=1;
    for j=1:(length(s_space)+1)^2
        if pi_star(j) == i
            ag_state_action(i,cnt_i) = j;
            cnt_i=cnt_i+1;
        end
...
ag_states_2 = ag_state_action;
%save('agreggated_states_n8_g9_infbits2', 'ag_states_2')



 for i=1:25
     act_presence(i) = length(find(ag_states_2(i,:)>0));
 end
...
save('agreggated_states_n8_g22_infbits3_AbSAIC', 'ag_states')
...
% G=zeros(nn,nn);
% for i=1:nn
%    G(nn-i+1,:) =agr_st(1+nn*(i-1):i*nn);
% end

% agr_st_new=zeros(nn^2,1);
% for i=1:nn^2
%     [agr_st_new(i),~]=find(ag_states_median_new==i) ;
% end

% T=zeros(nn,nn);
% for i=1:nn
%    T(nn-i+1,:) =agr_st_new(1+nn*(i-1):i*nn);
% end
% figure
% imagesc(T)

%Evaluation of Approxiamtion in eq. (\ref{lemma 1 - equation})
val_func=zeros(n^2,1);

for ii=1:n^2
    val_func(ii)=max(qp_table(mps_calc([21;ii],8),:));
end
...
function [pi_1,pi_2] = compute_pi(q_table, goal_set, n)
    pi_1 = zeros(n^2,n^2,3);
    pi_2 = zeros(n^2,1);
    joint_act_ind = 0;
    dec_joint_act_ind = zeros(2,1);
    ...
    for kk = 1: n*n
        ...
        for ll = 1:n*n
            ...
            ps_ind = int16(mps_calc([kk,ll],n));
            [~, joint_act_ind] = max(q_table(ps_ind,:)); %encoded joint action index
            dec_joint_act_ind = pa_calc(joint_act_ind); %decoded joint action indexes
            pi_1(kk,ll,1) = dec_joint_act_ind(1);
            pi_1(kk,ll,2) = dec_joint_act_ind(2);
            pi_1(kk,ll,3) = joint_act_ind;
            pi_2(ps_ind)  = joint_act_ind;
        end
