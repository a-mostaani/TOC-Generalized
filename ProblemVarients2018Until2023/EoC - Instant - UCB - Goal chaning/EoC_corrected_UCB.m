function [rew,qp_table,qc_table,counter]=EoC_corrected_UCB(scen,n,noa,ns,bits,best_rew,~,goal_set,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,end_learn,update_tables)
        ...
        for i=1:ns
            ...
            %Here we select the location uniformly from the non-terminal locations of the greed                    
             ...
             valid_loc=zeros(n*n-1,1);
                for ii=1:n*n
                    if ii~=goal_set && flag_l==0
                        valid_loc(ii)=ii;
                    elseif ii~=goal_set && flag_l==1
                        valid_loc(ii-1)=ii;    
                    else
                        flag_l=1;
                    end
