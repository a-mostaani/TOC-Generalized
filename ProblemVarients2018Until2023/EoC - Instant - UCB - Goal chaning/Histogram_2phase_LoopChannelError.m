...
en=1;               %in this particular function you can only choose en=1
                    %other values don't work well
                    %number of epochs
...
for k=1:ep
    ...
    for i=1:en
        %learning phase
        [learning_rew(:,i),qp,qc,epoch_counter_learn(:,i)]=EoC_par_function_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p_learn,end_learn_learn,update_tables);

        %execution phase
        [epoch_rew(:,i),epoch_NC_table(i,:,:,:),epoch_NE_table(i,:,:,:,:)]=Hist_EoC_par_function_UCB(scen,n,noa,ns_exec,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc,qp,bsc_p_exec,end_learn_exec,0);
...
total_count_1=sum(sum(sum(epoch_NC_table(1,1,:,:)))); %calculating total number of times all action-states are visited
epoch_NC_table_1=epoch_NC_table/total_count_1;    %normalizing the counted values of state-actions

%calculating the entropy of enivronment action probability dist
p_ae_1=zeros(2^bits,1);
H_ae_1=0;
for i=1:5
    p_ae_1(i)=sum(sum(epoch_NE_table(1,1,:,:,i)));
    H_ae_1=H_ae_1+p_ae_1(i)*log(p_ae_1(i));
end
H_ae_1=-H_ae_1;

%calculating the entropy of enivronment action probability distribution
p_ae_2=zeros(2^bits,1);
H_ae_2=0;
for i=1:5
    p_ae_2(i)=sum(sum(epoch_NE_table(1,2,:,:,i)));
    H_ae_2=H_ae_2+p_ae_2(i)*log(p_ae_2(i));
end
H_ae_2=-H_ae_2;

%calculating the entropy of state-actions probability distribution: Agent#1
p_seae_1=epoch_NE_table(1,1,:,:,:)/sum(sum(sum(epoch_NE_table(1,1,:,:,:))));
p_se_ae_1=sum(p_seae_1(1,1,:,:,:),4);
p_se_ae_1=squeeze(p_se_ae_1);
p_seae_1=squeeze(p_seae_1);
p_actions_norm_1=p_se_ae_1;
for i=1:n^2-1
    p_actions_norm_1(i,:)=p_actions_norm_1(i,:)/sum(p_actions_norm_1(i,:));
end

%calculating the entropy of state-actions probability distribution: Agent#2
p_seae_2=epoch_NE_table(1,2,:,:,:)/sum(sum(sum(epoch_NE_table(1,2,:,:,:))));
p_se_ae_2=sum(p_seae_2(1,1,:,:,:),4);
p_se_ae_2=squeeze(p_se_ae_2);
p_seae_2=squeeze(p_seae_2);
p_actions_norm_2=p_se_ae_2;
for i=1:n^2-1
    p_actions_norm_2(i,:)=p_actions_norm_2(i,:)/sum(p_actions_norm_2(i,:));
end
...
H_se_1=0;
for i=1:n^2-1
    p_se_1(i)=sum(epoch_NC_table_1(1,1,i,:));
...
H_ac_1=0;
for i=1:2^bits
    p_ac_1(i)=sum(epoch_NC_table_1(1,1,:,i));
    H_ac_1=H_ac_1+p_ac_1(i)*log(p_ac_1(i));
end
H_ac_1=-H_ac_1;

%calculating the entropy of state-actions probability distribution
p_seac_1=epoch_NC_table_1(1,1,:,:);
H_seac_1=-sum(sum(epoch_NC_table_1(1,1,:,:).*log(epoch_NC_table_1(1,1,:,:))));

minf_seac_1=H_se_1+H_ac_1-H_seac_1


%calculating entropy for 2nd agent
total_count_2=sum(sum(sum(epoch_NC_table(1,2,:,:))));
...
H_se_2=0;
for i=1:n^2-1
    p_se_2(i)=sum(epoch_NC_table_2(1,2,i,:));
...
H_ac_2=0;
for i=1:2^bits
    p_ac_2(i)=sum(epoch_NC_table_2(1,2,:,i));
...
p_se_1_grid=flipud(p_se_1_grid);
figure
imagesc(p_se_1_grid);

%State distribution of agent_2
p_se_2_grid=zeros(n,n);
for i=1:n
    for j=1:n
        p_se_2_grid(i,j)=p_se_2((i-1)*n+j);
    end
end
p_se_2_grid=flipud(p_se_2_grid);
figure
imagesc(p_se_2_grid);
