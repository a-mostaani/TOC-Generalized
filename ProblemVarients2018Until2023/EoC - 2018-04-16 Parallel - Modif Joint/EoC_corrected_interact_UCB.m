function [rew,q_table,counter]=EoC_corrected_interact_UCB(scen,n,noa,ns,bits,best_rew,~,~,gamma,~,~,cs,~,~,~,rew,~,counter,q_table,bsc_p,end_learn,update_tables)
...
%use new functions of cpolicy costumized cupdate customized and ...

N_table=0.001*ones(2,n^2,2^bits,5,2^bits);

% This program contains 4 modifications marked with labels " %modified  " 
%%modified 1
%                       % we want to make sure that the communication
%                       action 2^B will not be selected so the only time an
%                       agent receives 2^B is the starting time step
N_table(:,:,:,:,2^bits)=20000*ones(2,n^2,2^bits,5); 
...
%             end
            ...
            switch scen
                ...
                case 2
                    ca=ps;
                case 3
                    ca=randi(2,bits,noa)-ones(bits,noa);
            end
        ...
        %episode run    
            while 3==3    
                ...
                %SELECT BOTH ACTIONs AND UPDATE BOTH STATES
                [pa,ca] = jpolicy_customized_nbits_UCB(ps,cs,scen,tau,q_table,N_table,ucb_counter,ns,end_learn);
                %                                     (ps,cs,scen,tau,q_table,NE_table,ucb_counter,~,~)
                
                
                for k=1:noa %modified 4
                    if bi2de(transpose(ca(:,k)))+1==2^bits
                        error('invalid ca is selected')
                    end
                end
                %Update UCBC counter
                for k=1:2
                    if counter(i)>1 %modified 3
                        N_table(k,ps(k),bi2de(transpose(cs(:,k)))+1,pa(k),bi2de(transpose(ca(:,k)))+1)=N_table(k,ps(k),bi2de(transpose(cs(:,k)))+1,pa(k),bi2de(transpose(ca(:,k)))+1)+1;                
                    end
                end
...
%                                                   %(ps,cs,scen,tau,q_table,counter,ns,end_learn)
%                 %Update UCBE counter
%                 for k=1:2
...
% %                             [qp_table] = pupdate_customized_nbits_2(ps,last_ps,cs,cs,scen,pa,temp_rew,rew_winner,tau,qp_table);
                            for k=1:noa
                                ...
                                %q_table(k,last_ps(k),bi2de(transpose(squeeze(last_cs(k,1,:))))+1,pa(k))=median(qp_table(k,last_ps(k),bi2de(transpose(squeeze(last_cs(k,1,:))))+1,:));
                                q_table(k,last_ps(k),bi2de(transpose(last_cs(:,k)))+1,pa(k),bi2de(transpose(last_cs(:,k)))+1);
                            end
                        end
                        break
                    end
...
% % %                 end
                ...
                % TERMINAL STATE CHECK AND REWARD ALLOCATION                   
...
% %                 end
                ...
                %no update
                    ...
                    if update_tables==1
                        [q_table] = jupdate_customized_nbits_2(ps,last_ps,cs,last_cs,scen,pa,ca,temp_rew,rew_winner,tau,q_table);
                    end
                end
