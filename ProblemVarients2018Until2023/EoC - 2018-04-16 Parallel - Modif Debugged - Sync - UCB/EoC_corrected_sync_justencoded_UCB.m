function [rew,qp_table,qc_table,counter]=EoC_corrected_sync_justencoded_UCB(scen,n,noa,ns,bits,inf_bits,best_rew,worst_rew,~,gamma,~,~,cs,~,~,~,rew,~,counter,qc_table,qp_table,bsc_p,end_learn,update_tables)
...
% CS is the argument for the Q^e and 
% CA_TILDE is the value generated by the function Q^c
...
%             end
        ...
        %episode run    
            while 3==3    
                ...
                %select communication action (before applying channel coding)
                ...
                %select position action (fix_me: policy needs to be selected based on the decoded message)
                [pa] = ppolicy_customized_nbits_UCB(ps,cs,scen,tau,qp_table,NE_table,ucb_counter,ns,end_learn);
                
                %Update UCBE counter (fix_me: cs needs to be replaced by the decoded message)
                for k=1:2
