function [rew,qp_table,qc_table,counter]=EoC_corrected_sync_UCB(scen,n,noa,ns,bits,best_rew,~,~,gamma,~,~,cs,~,~,~,rew,~,counter,qc_table,qp_table,bsc_p,end_learn,update_tables)
...
%                       agent receives 2^B is the starting time step
 NC_table(:,:,2^bits)=20000*ones(2,n^2); 
...
%             end
        ...
        %episode run    
            while 3==3    
                ...
                for j=1:noa
                    if ps(j)==n^2 %|| counter(i)==1%%modified 2
                        pa(j)=5;
                    end
                end
