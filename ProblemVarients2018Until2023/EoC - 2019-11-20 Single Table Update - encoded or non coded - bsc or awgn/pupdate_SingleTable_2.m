function [qp_table] = pupdate_SingleTable_2(ps,last_ps,cs,last_cs,scen,pa,temp_rew,rew_winner,tau,qp_table)
%General help
...
%                         else
                             ...
                             %statement should be changed: #noa
                                                  ...
                                                  %          |
                                                  %          |
                              qp_table(last_ps(i),data_received(i,1),pa(i))=(1-alpha)*qp_table(last_ps(i),data_received(i,1),pa(i))+alpha*(0+gamma*max(qp_table(ps(i),bi2de(transpose(squeeze(cs(i,1,:))))+1,:)));
%                         end
                    end
               elseif temp_rew~=0 && length(rew_winner)==1
                    for i=1:noa
                         if ismember(i,rew_winner)
                              qp_table(last_ps(i),data_received(i,1),pa(i))=(1-alpha)*qp_table(last_ps(i),data_received(i,1),pa(i))+alpha*(temp_rew);
