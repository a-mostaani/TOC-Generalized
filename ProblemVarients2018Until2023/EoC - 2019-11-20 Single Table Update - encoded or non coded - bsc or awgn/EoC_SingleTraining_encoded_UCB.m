function [rew,qp_table,qc_table,counter]=EoC_SingleTraining_encoded_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,ch_type,end_learn,update_tables)
...
if update_tables==1
    qp_table=0.02*ones(n^2,2^inf_bits,5);
    qc_table=0.02*ones(n^2,2^inf_bits);
    NE_table=0.001*ones(n^2,2^inf_bits,5);
    NC_table=0.001*ones(n^2,2^inf_bits);
end
...
stuck_counter=0;
        ...
        %episode run    
            while 3==3    
                ...
                [ca_tilde] = cpolicy_SingleTable_UCB(ps,scen,tau,qc_table,NC_table,ucb_counter,ns,end_learn);
                                                                
                %Update UCBC counter
                NC_table(ps(1),bi2de(ca_tilde(1,:))+1)=NC_table(ps(1),bi2de(ca_tilde(1,:))+1)+1;
                NC_table(ps(2),bi2de(ca_tilde(2,:))+1)=NC_table(ps(2),bi2de(ca_tilde(2,:))+1)+1;
                
                %Encode the Communication Message
                ca = encode(ca_tilde,bits,inf_bits,'cyclic/fmt');
                ...
                if counter(i)~=1  && update_tables==1 
                    ...
                    [qp_table] = pupdate_SingleTable_2(ps,last_ps,cs,last_cs,scen,pa,temp_rew,rew_winner,tau,qp_table); 
                end
        
                %SELECT POSITION ACTION AND UPDATE POSITON STATE
                %select position action 
                [pa] = ppolicy_SingleTable_UCB(ps,cs,scen,tau,qp_table,NE_table,ucb_counter,ns,end_learn);
        
                %Update UCBE counter
                for k=1:2
                    NE_table(ps(k),bi2de(transpose(squeeze(cs(k,1,:))))+1,pa(k))=NE_table(ps(k),bi2de(transpose(squeeze(cs(k,1,:))))+1,pa(k))+1;
                end
...
% %                             [qp_table] = pupdate_customized_nbits_2(ps,last_ps,cs,cs,scen,pa,temp_rew,rew_winner,tau,qp_table);
                            for k=1:noa
                                ...
                                qp_table(last_ps(k),bi2de(transpose(squeeze(last_cs(k,1,:))))+1,pa(k))=median(qp_table(last_ps(k),bi2de(transpose(squeeze(last_cs(k,1,:))))+1,:));
                            end
                        end
                        break
                    end
...
% %                 end
                ...
                %no update
                    ...
                    if update_tables==1
                        [qc_table] = cupdate_SingleTable(ps,last_ps,ca_tilde,scen,temp_rew,rew_winner,qc_table);
                    end
                end
                ...
                if ter >=1 
                    ...
                    %updating position table:
                    if update_tables==1
                        [qp_table] = pupdate_SingleTable_2(ps,last_ps,cs,cs,scen,pa,temp_rew,rew_winner,tau,qp_table);
                    end
                        term_hist(i)=ter;
                    break
                end
