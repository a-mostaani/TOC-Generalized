...
ch_type="bsc";
bsc_p_learn=0.0000001; %if the channel is awgn then bsc_p is snr(dB) otw its error probability of bsc channel
bsc_p_exec= 0.0000001;
...
qc_table=0.02*ones(n^2,2^bits);
qc=      0.02*ones(n^2,2^bits);
                    %qc3_table(n^2,:)=[2,2];
...
switch scen
   ...
   case 3
       qp_table=0.02*ones(n^2,2^bits,5);
       qp=      0.02*ones(n^2,2^bits,5);
end
...
for i=1:en
    %learning phase
    [learning_rew(:,i),qp,qc,epoch_counter_learn(:,i)]=EoC_SingleTraining_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p_learn,ch_type,end_learn_learn,update_tables);
    
    %execution phase
    %[epoch_rew(:,i)]=EoC_SingleTraining_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc,qp,bsc_p_exec,ch_type,end_learn_exec,0);
...
mean_rew_learn
hold on
plot(movmean(learning_rew(1:ns/50:ns),ns*.2))
learn_rew_avg=movmean(learning_rew,ns*.2);
plot(1:ns/25:ns,learn_rew_avg(1:ns/25:ns),'-d')
save('result_ucb1a05_codedsingle_rlcompression.mat', 'learning_rew', 'learn_rew_avg')
...
% The communication message generated at each location (locations are indices of the vector) 
