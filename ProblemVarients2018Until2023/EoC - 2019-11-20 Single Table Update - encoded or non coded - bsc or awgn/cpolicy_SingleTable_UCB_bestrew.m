function [ca] = cpolicy_SingleTable_UCB_bestrew(ps,scen,tau,qc_table,NC_table,ucb_counter,ns,end_learn,best_rew)
    ...
    case 3
        switch policy
            case "ucb"
                ...
                for i=1:noa
                    %greedy selection
                    ucb_func(:,i)=ucb_const*sqrt(log(ucb_counter+1)./NC_table(ps(i),:));
                    [~,ind]=max(squeeze(qc_table(ps(i),:))+transpose(ucb_func(:,i)));
                    ca(i,:)=zero_fix(de2bi(ind(randi(length(ind)))-1),bits); %ind(randi(length(ind))) is acually ind but because the max function might provide us with an array of max values and their indices, we should randomly select amongst them
                end
            case "ep_greedy"
                for i=1:noa
                    rr=rand;
