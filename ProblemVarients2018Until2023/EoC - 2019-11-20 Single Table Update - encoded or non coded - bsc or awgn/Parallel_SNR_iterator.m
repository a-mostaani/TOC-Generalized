...
ch_type="awgn";
bsc_p_learn=6; %if the channel is awgn then bsc_p is snr(dB) otw its error probability of bsc channel
bsc_p_exec=6;
...
learning_rew=zeros(ns,en,16); %where 16 stands for the number of SNR values -8:8 we evaluate the system over
...
%To Do: Learning reward should caputre the SNR rate as well
tic
bsc_p_learn=8;
for kk=1:16
    %Adaptive number of tranining epochs for different SNRs
    if bsc_p_learn <= 2
        en=fix(6-0.5*bsc_p_learn);
    else
        en=3;
    end
    ...
    for i=1:en
        %learning phase
        %note that kk=16 relates to SNR=-8dB and kk=1 to SNR=8dB
        [learning_rew(:,i,kk),qp,qc,epoch_counter_learn(:,i)]=EoC_SingleTraining_UCB(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p_learn,ch_type,end_learn_learn,update_tables);
...
mean_rew_learn
hold on
plot(movmean(learning_rew(1:ns*.8),ns*.1))