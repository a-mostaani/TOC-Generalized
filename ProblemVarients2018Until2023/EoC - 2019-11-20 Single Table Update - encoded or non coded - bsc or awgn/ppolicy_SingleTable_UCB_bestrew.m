function [pa] = ppolicy_SingleTable_UCB_bestrew(ps,cs,scen,tau,qp_table,NE_table,ucb_counter,ns,end_learn,best_rew)
    ...
    case 3
        switch policy
            case "ucb"
                ucb_func=zeros(5,2);
                for i=1:noa
                    ucb_func(:,i)=ucb_const*sqrt(log(ucb_counter+1)./NE_table(ps(i),bi2de(zero_fix(cs(i,1,:),bits))+1,:));
                    [~,pa(i)]=max(squeeze(qp_table(ps(i),bi2de(zero_fix(cs(i,1,:),bits))+1,:))+ucb_func(:,i));
                end
            case "ep_greedy"
                for i=1:noa
                    rr=rand;
