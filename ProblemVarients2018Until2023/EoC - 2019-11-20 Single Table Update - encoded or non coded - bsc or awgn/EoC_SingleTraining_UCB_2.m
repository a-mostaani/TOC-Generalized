function [rew,qp_table,qc_table,counter]=EoC_SingleTraining_UCB_2(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,ch_type,end_learn,update_tables)
...
NE_table=0.001*ones(n^2,2^bits,5);
NC_table=0.001*ones(n^2,2^bits);
...
stuck_counter=0;
        ...
        %episode run    
            while 3==3    
                ...
                [ca] = cpolicy_SingleTable_UCB(ps,scen,tau,qc_table,NC_table,ucb_counter,ns,end_learn,best_rew);
                                                                
                %Update UCBC counter
                NC_table(ps(1),bi2de(ca(1,:))+1)=NC_table(ps(1),bi2de(ca(1,:))+1)+1;
                NC_table(ps(2),bi2de(ca(2,:))+1)=NC_table(ps(2),bi2de(ca(2,:))+1)+1;
                ...
                [pa] = ppolicy_SingleTable_UCB(ps,cs,scen,tau,qp_table,NE_table,ucb_counter,ns,end_learn,best_rew);
...
% %                 end
                ...
                %no update
                    ...
                    if update_tables==1
                        [qc_table] = cupdate_SingleTable(ps,last_ps,ca,scen,temp_rew,rew_winner,qc_table);
                    end
                end
