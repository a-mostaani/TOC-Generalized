function [rew,qp_table,qc_table,counter]=EoC_corrected_position_lloyds_UCB_2(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,end_learn,update_tables,partition_1,partition_2,inf_bits,goal_set)
...
partition=[partition,[n^2+.1;n^2+.1]];
        ...
        for i=1:ns
            ...
            switch scen
                ...
                case 2
                    ca=ps;
                case 3
                    ca=randi(2,noa,inf_bits)-ones(noa,inf_bits);
            end
        ...
        %episode run    
            while 3==3    
                ...
                if bits>inf_bits
                        for j=1:noa
                            for k=1:2^inf_bits
                                if ps(j,:)<partition(j,k)
                                    ca(j,:)=de2bi(k-1,inf_bits);
                                    break
                                end
                            end
                        end
