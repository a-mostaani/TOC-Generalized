...
bsc_p_learn=0.000000001; 
bsc_p_exec= 0.000000001;
...
load('sourcecoded_states_g22_n8_br10', 'ps_hist')
...
for i=1:en
    %learning phase
    %[learning_rew(:,i),qp,qc,epoch_counter_learn(:,i),ps_hist]=EoC_corrected_position_UCB_2(scen,n,noa,ns,bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p_learn,end_learn_learn,update_tables,goal_set);
    
    %lloyds algorithm to extract 
    [partition_1,codebook_1]=lloyds(ps_hist(1,:),2^inf_bits);
    [partition_2,codebook_2]=lloyds(ps_hist(2,:),2^inf_bits);
    
    %retrain the policy using compact observations
    [learning_rew_lloyds(:,i)]                                =EoC_corrected_position_lloyds_UCB_2(scen,n,noa,ns,inf_bits,best_rew,worst_rew,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p_learn,end_learn_learn,update_tables,partition_1,partition_2,inf_bits,goal_set);
...
%save('sourcecoded_states_g22_n8_br10', 'ps_hist')
...
mean_rew_learn_lloyds=mean(mean(learning_rew_lloyds(fix(ns*end_learn_learn+1):end,:)));
...
% hold on
% plot(mean(movmean(learning_rew_lloyds,ns*0.1),2))
% legend('Without source coding','With source coding')

movmean_learning_rew=movmean(learning_rew_lloyds,2000);
