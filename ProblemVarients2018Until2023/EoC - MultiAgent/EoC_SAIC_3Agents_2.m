function [rew,qp_table,qc_table,counter,NE_table_emerged]=EoC_SAIC_3Agents_2(scen,n,noa,ns,bits,inf_bits,best_rew,worst_rew,goal_set,gamma,tau_k,ca,cs,pa,ps,ter,rew,temp_rew,counter,qc_table,qp_table,bsc_p,end_learn,update_tables,policy,fixed_aggregation,batch_number)
 if ~exist('policy')
    prompt = "What is the policy? Write it as a string! ";
    policy = input(prompt);
 end
...
%load('agreggated_states_n3_g9_infbits2_realSAIC','ag_states_median');
%load('agreggated_states_n3_g9_infbits2_realSAIC_en5', 'batch_ag_states_median');
%load('agreggated_states_n4_g11_infbits2_ESAIC','ag_states')
...
%save('agreggated_states_n3_g9_infbits2_realSAIC', 'batch_ag_states_median')
...
monit = zeros(1,ns);
monit_count = 0;
if fixed_aggregation ==1
    ag_states = ag_states_median;
else
    ag_states = batch_ag_states_median(:,:,batch_number);
end
...
%                         end
                ...
                if counter(i)~=1  && update_tables==1 
                    ...
                    [qp_table] = pupdate_customized_nbits_ma(ps,last_ps,cs,last_cs,scen,pa,temp_rew,rew_winner,tau,qp_table); 
...
%                        monit(monit_count) = qp_table(3,14,4,3); 
%                     end
                end
                ...
                policy = "ep_greedy";
                [pa] = ppolicy_customized_nbits_UCB_bestrew_ma(ps,cs,scen,tau,qp_table,NE_table,ucb_counter,ns,end_learn,best_rew,policy,i);
...
%                     end
                            ...
                            for k=1:noa
                                ...
                                qp_table(k,last_ps(k),bi2de(last_cs_new(k,:))+1,pa(k))=median(qp_table(k,last_ps(k),bi2de(last_cs_new(k,:))+1,:));
                            end
                        end
                        break
                    end
